...
def archives(request, year, month):
    ...
    post_list=Post.objects.filter(created_time__startswith=date)
...
class ArchiveView(ListView):
    ...
    def get_queryset(self):
        ...
        date = str(year) + '-' + (month if int(month) >= 10 else str(0) + str(month))
        return super(ArchiveView, self).get_queryset().filter(created_time__startswith=date)
...
class CategoryView(ListView):
    ...
    def get_queryset(self):
        ...
        return super(CategoryView, self).get_queryset().filter(category=name)
